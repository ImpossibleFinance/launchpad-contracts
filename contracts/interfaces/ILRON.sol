...
interface ILRON {
  ...
  error ExceedMaxLockUntilAllowance(uint256 maxUntilAllowed, uint256 requestedUntil);
  ...
  error InsufficientLockedBalance(address account, uint256 balance, uint256 needed);
  /// @dev Throws if the lockable allowance is insufficient.
  error InsufficientLockableAllowance(address locker, uint256 current, uint256 spent);
  ...
  event VaultBalanceUpdated(address indexed locker, uint256 prevBalance, uint256 newBalance);
  /**
  ...
  event Lock(address indexed account, address indexed locker, uint256 amount, uint256 lockedAt, uint256 lockedUntil);
  /// @dev Emitted when the user approves the lock.
  event LockApproval(address indexed account, address indexed locker, uint256 amount, uint256 maxUntil);
   ...
   * Emits a {Lock} event.
   */
  function lockFrom(address account, uint96 amount, uint40 until) external returns (bool);
   ...
   */
  function approveLock(address locker, uint96 amount, uint40 until) external returns (bool);

  /**
   * @notice Get lockable allowance data of the locker from the account.
   */
  function lockableAllowance(address account, address locker) external view returns (LockableAllowance memory);
   ...
   */
  function totalDeposited(address account) external view returns (uint256 deposited);
